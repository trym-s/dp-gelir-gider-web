...
import { getIncomes, createIncome, uploadIncomesExcel, importValidatedIncomes, uploadDubaiIncomesExcel, exportIncomes } from "../../api/incomeService";
...
const getStatusTag = (record) => {
    ...
    const { status, timeliness_status } = record;
    const statusMap = {
    ...
    const timelinessMap = {
        LATE: { color: 'magenta', text: 'Geç Ödendi' },
        EARLY: { color: 'cyan', text: 'Erken Ödendi' },
        ON_TIME: { color: 'blue', text: 'Vaktinde Ödendi' },
    };
    const statusInfo = statusMap[status] || { color: 'default', text: status };
    const timelinessInfo = timeliness_status ? timelinessMap[timeliness_status] : null;
    return (
        <div>
            <Tag color={statusInfo.color}>{statusInfo.text}</Tag>
            {timelinessInfo && <Tag color={timelinessInfo.color}>{timelinessInfo.text}</Tag>}
        </div>
    );
};
...
/** YENİ: Para birimine göre biçimlendirme (TRY, USD, EUR, GBP, AED) */
const formatMoney = (amount, currency) => {
    const num = Number(amount ?? 0);
    try {
        return num.toLocaleString(undefined, { style: 'currency', currency: currency || 'TRY' });
    } catch {
        return `${num} ${currency}`;
    }
};
...
export default function IncomeList() {
    ...
    const [sortInfo, setSortInfo] = useState({ field: 'issue_date', order: 'descend' });
    ...
    const refreshPageData = useCallback(() => {
        ...
        setSortInfo({ field: 'issue_date', order: 'descend' });
    ...
    const {
        ...
        setEditableRows,
        setIsModalVisible
    } = useExcelImport(uploadIncomesExcel, importValidatedIncomes, refreshPageData);
    ...
    const handleDubaiUpload = async ({ file }) => {
        ...
        try {
            ...
            if (!Array.isArray(results)) {
                message.error("Sunucudan geçersiz bir yanıt formatı alındı."); return;
            }
    ...
    const handleTableChange = (pagination, filters, sorter) => {
        ...
        setSortInfo({ field: sorter.field || 'issue_date', order: sorter.order || 'descend' });
    };
    ...
    const handleExport = async (exportFilters = {}) => {
        ...
        try {
            ...
            if (formattedFilters.date_range && formattedFilters.date_range.length === 2) {
                formattedFilters.date_start = dayjs(formattedFilters.date_range[0]).format('YYYY-MM-DD');
    ...
    const mainColumns = [
        ...
        { title: "Vergi Numarası", dataIndex: ["customer", "tax_number"], key: "tax_number" },
        {
            ...
            sorter: true,
            align: 'right',
            render: (val, record) => formatMoney(val, record.currency)
        },
        {
            title: "Tahsil Edilen",
    ...
    return (
        <div style={{ padding: 24 }}>
            <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
                ...
                <div>
                    {/* --- GÜNCELLEME: Butonun onClick'i artık sadece pencereyi açıyor --- */}
                    <Button
            ...
            <Modal
                title="Gelir Excel Yükleme Sonuçları"
                open={isModalVisible}
                onCancel={closeUploadModal}
                width={1300}
                footer={[
                    <Button key="back" onClick={closeUploadModal}>Kapat</Button>,
                    <Button key="submit" type="primary" loading={importLoading} onClick={handleSaveImports} icon={<SaveOutlined />}>Doldurulanları İçe Aktar</Button>
                ]}
            >
            ...
            {/* --- YENİ: Dışa Aktarma Seçenekleri Penceresi --- */}
            <Modal
            ...
            >
                ...
                <Form form={exportFilterForm} onFinish={handleExport} layout="vertical" style={{ marginTop: 24 }}>
                    <Row gutter={[16, 16]}>
                        <Col span={12}>
                            <Form.Item name="search_term" label="Fatura No / İsim Ara">
                                <Input.Search placeholder="Aranacak metni girin..." allowClear />
                            </Form.Item>
                        </Col>
                        <Col span={12}>
                            <Form.Item name="date_range" label="Tarih Aralığı">
