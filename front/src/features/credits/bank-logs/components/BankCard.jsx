...
const cardStyles = {
    container: {
        ...
        // GÖRÜNÜM MODU İÇİN GRID YAPISI
        gridTemplateColumns: 'minmax(0, 1.5fr) 180px repeat(5, 1fr)',
    },
    // --- DÜZENLEME MODU İÇİN EK STİLLER ---
    containerEditing: {
        borderColor: 'var(--primary-color-40)',
        boxShadow: '0 4px 8px var(--shadow-color-15)',
        // DÜZENLEME MODUNDA YENİ GRID YAPISI:
        // 1. Sütun: Banka adı. En az 150px ol, gerekirse büyü. Bu, ezilmeyi önler.
        // Kalan 5 sütun: Geriye kalan alanı eşit paylaş.
        gridTemplateColumns: 'minmax(150px, 1fr) repeat(5, 1fr)',
    },
    bankInfo: {
        display: 'flex',
        alignItems: 'center',
        gap: 'var(--spacing-sm)',
        minWidth: 0, // Bu, flexbox'ın içindeki elemanların küçülmesine izin verir.
    },
    logo: {
        width: 'auto',
        height: '20px',
        objectFit: 'contain',
        flexShrink: 0, // Logonun küçülmesini engelle
    },
    bankName: {
        fontSize: '1rem',
        fontWeight: '600',
        ...
        whiteSpace: 'nowrap',
        overflow: 'hidden',
        textOverflow: 'ellipsis', // Taşarsa "..." ile göster
    },
    totalHighlight: {
        display: 'flex',
        alignItems: 'center',
...
export function BankCard({ bankData, editMode, onBalanceChange, currentRates, bankLogoMap }) {
    ...
    // ... (useMemo ve getTooltipContent fonksiyonları aynı kalacak, değişiklik yok)
    const totalInTry = useMemo(() => {
        ...
        const total = (parseFloat(log.amount_try) || 0) + (parseFloat(log.amount_usd) || 0) * parseFloat(usdRate) + (parseFloat(log.amount_eur) || 0) * parseFloat(eurRate) + (parseFloat(log.amount_aed) || 0) * parseFloat(aedRate) + (parseFloat(log.amount_gbp) || 0) * parseFloat(gbpRate);
        return total;
    }, [bankData.log, currentRates]);
    ...
    return (
        <div style={containerStyle}>
            ...
            {/* Toplam Bakiye - Sadece GÖRÜNÜM modunda gösterilecek */}
            {!editMode && (
