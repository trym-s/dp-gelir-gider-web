...
import { getLoans, createLoan, updateLoan, deleteLoan, getLoanTypes, createLoanType } from '../../api/loanService';
...
export default function BankLoans() {
 ...
 const fetchLoans = async () => {
  try {
    const loans = await getLoans(); // ← loans burada tanımlanıyor
  ...
  const handleAddLoan = async () => {
    try {
      ...
      if (editMode) {
        await updateLoan(payload.id, payload);
      } else {
  ...
  const handleAddSelectItem = async () => {
    ...
    if (newSelectType === 'bank') {
      ...
      try {
        const result = await createLoanType({ name: newSelectItem }); // 👈 backend'e POST
